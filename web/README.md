...
1. Choose a color and speed, then tap **Blink at speed N**. The blink command is sent as usual; the light starts blinking at that speed value. To skip the slider, type a speed into the number box in the speed row and press Enter — the value clamps to the 0–255 firmware range and the blink is sent immediately.
2. Tap **Start camera** and center the lit lightstick inside the alignment circle; a dim background helps. The circle is adjustable like the Capture Lab guide (drag to move, pinch or Ctrl+scroll to resize), and only the luma inside it feeds the detector — background light outside the circle is ignored. The frame-brightness meter shows the detected on/off signal, and the stats show the number of blinks, the latest period, and the median rate in blinks/min. Detection uses brightness only, so a dark blink color is hard to measure (a warning appears). The **Blink threshold** slider (default 15%) sets how far the brightness must move past its rolling midpoint to count as a blink: lower counts weaker changes, higher rejects noise and stray light — it applies live, on the next frame, and is locked while a sweep runs.
